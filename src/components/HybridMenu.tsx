import React, { useCallback, ReactNode, MutableRefObject } from 'react';
...
interface HybridMenuProps {
  renderMenuItem?: (item: MenuItem, idx: number) => ReactNode;
  menuItems?: MenuItem[];
  onMenuOpen?: () => void;
  onMenuClose?: () => void;
  ...
  onBack?: () => void;
  onOverlayNavigate?: (route: string) => void;
}

function DesktopMenu(props: HybridMenuProps & { splashOpen?: boolean; onSplashClose?: () => void; audioRef?: MutableRefObject<HTMLAudioElement | null>; }) {
  const {
    renderMenuItem,
  ...
  const handleSplashOpenWithAudio = useCallback((audio?: string) => {
