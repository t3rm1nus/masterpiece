...
interface MaterialRecommendationCardProps {
  recommendation: any; // Usar any para evitar conflictos con el tipo Item
  onClick?: (recommendation: any) => void;
  showCategory?: boolean;
  showSubcategory?: boolean;
  sx?: any; // Usar any para evitar conflictos con SxProps
  className?: string;
  actions?: ReactNode;
}
 ...
 */
const MaterialRecommendationCard = memo<MaterialRecommendationCardProps>(({
  recommendation,
  onClick,
  showCategory = true,
  ...
  const title = processTitle(recommendation.title || (recommendation as any).name, lang);
  const description = processDescription(recommendation.description, lang);
  const truncateDescription = (desc: string, category: string, maxLength = 150): string => {
    if (!desc) return '';
  ...
  const cardSx = (theme: Theme): any => {
    ...
    return {
      ...
      boxShadow: (recommendation as any).masterpiece ? theme.shadows[4] : theme.shadows[2],
      ...
      border: (recommendation as any).masterpiece ? '2px solid #ffd700' : 'none',
      background: (recommendation as any).masterpiece
        ? (theme.palette.mode === 'dark' 
          ? 'linear-gradient(135deg, #2a2600 60%, #333300 100%)'
          : 'linear-gradient(135deg, #fffbe6 60%, #ffe066 100%)')
        : ((sx as any)["--card-gradient"] || getCategoryGradient(recommendation.category)),
      ...sx
    };
  };

  const badgeSx = (theme: Theme): any => ({
    position: 'absolute',
    top: 0,
    right: 0,
    zIndex: 10,
  ...
  const categoryChipSx = (cat: string): SxProps<Theme> => ({
  ...
  const subcategoryChipSx = (cat: string): SxProps<Theme> => ({
    fontSize: '0.68rem',
    ...
    >
      ...
      <CardContent sx={{ position: 'relative', overflow: 'visible' }}>
        ...
        <Box sx={{ marginBottom: '8px', width: '100%', display: 'flex', justifyContent: 'flex-start' }}>
          {showCategory && (
            <Chip
              icon={getCategoryIcon(recommendation.category) as React.ReactElement}
